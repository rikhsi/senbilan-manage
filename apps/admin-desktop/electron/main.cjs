...
/**
 * Electron main process for admin-desktop.
 * Security: contextIsolation + sandbox + no nodeIntegration.
 * Renderer is the Angular admin app (dev URL or packaged dist).
 */
...
const createWindow = () => {
  ...
  if (isDev) {
    ...
    const indexHtml = path.join(__dirname, '../../../dist/apps/admin/browser/index.html');
